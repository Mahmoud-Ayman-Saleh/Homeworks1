    ...
    private:
        ...
        {
            if (is_valid(r, c, grid) && (r == 0 || r == grid.size()-1 || c == 0 || c == grid[0].size()-1) && grid[r][c] == x)
                return true;
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (!is_valid(nr, nc, grid) || grid[nr][nc] != x) return true;
            }
        ...
        void dfs(int r, int c, vector<vector<int>>& grid, vector<vector<bool>> &visited, int x, int color)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                dfs(r + dr[i], dc[i] + c, grid, visited, x, color);
            }
